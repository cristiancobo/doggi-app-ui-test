...
describe("Given a created dog breed", () => {
    ...
    describe("When the user wants to edit the dog breed", () => {
        ...
        it("Then the dog breed should be listed with the updated attributes", () => {
            ...
            cy.get(`[data-testid="e2e-dog-card-height-${new_dog_breed.name}"]`).should("have.text", `Height: ${updatedDog.height}`);
            cy.get(`[data-testid="e2e-dog-card-weight-${new_dog_breed.name}"]`).should("have.text", `Weight: ${updatedDog.weight}`);
            cy.get(`[data-testid="${new_dog_breed.name}"] > .MuiCardHeader-root > .MuiCardHeader-content > .MuiCardHeader-subheader`).should("have.text", `Life Expectancy: ${updatedDog.lifeExpectancy}`);
        });
    });
});